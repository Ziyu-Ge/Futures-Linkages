...
| 方法 | 口径 |
| --- | --- |
| 滞后相关 | $\operatorname{corr}(\mathrm{leader}_t,\mathrm{follower}_{t+\ell})$ 与反方向差值 $\mathrm{lead\_edge}$ |
...
$$
\rho_{\mathrm{lag}}(i \to j,k) = \operatorname{corr}(r_{i,t}, r_{j,t+k})
$$

反方向为：

$$
\rho_{\mathrm{lag}}(j \to i,k) = \operatorname{corr}(r_{j,t}, r_{i,t+k})
$$
...
$$
\mathrm{factor}_{i,t}
= \frac{1}{N_g-1}\sum_{\substack{j\in g\\j\ne i}} r_{j,t}
$$
...
$$
\begin{aligned}
\beta_i &= \frac{\operatorname{Cov}(r_i,\mathrm{factor}_i)}{\operatorname{Var}(\mathrm{factor}_i)} \\
\alpha_i &= \operatorname{mean}(r_i) - \beta_i\,\operatorname{mean}(\mathrm{factor}_i)
\end{aligned}
$$
...
&= \operatorname{corr}(\varepsilon_{i,t},\varepsilon_{j,t+k}) \\
...
= \frac{\#\{(t,w,k):\mathrm{lead\_strength}(i \to j,t,w,k)>0\}}{M}
$$
...
\mathrm{var\_score}=
\begin{cases}
1.0, & p_{\mathrm{VAR}} < 0.05 \land (A_\ell)_{j,i} > 0 \\
0.5, & p_{\mathrm{VAR}} < 0.05 \land (A_\ell)_{j,i} < 0 \\
